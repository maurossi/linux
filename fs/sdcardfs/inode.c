...
#if 0
static int sdcardfs_readlink(struct user_namespace *mnt_userns, struct dentry *dentry, char __user *buf, int bufsiz)
{
	int err;
	struct dentry *lower_dentry;
	struct path lower_path;
...
static int sdcardfs_permission_wrn(struct user_namespace *mnt_userns, struct inode *inode, int mask)
{
...
static int sdcardfs_permission(struct vfsmount *mnt, struct user_namespace *mnt_userns, struct inode *inode, int mask)
{
	int err;
	struct inode tmp;
...
static int sdcardfs_setattr_wrn(struct user_namespace *mnt_userns, struct dentry *dentry, struct iattr *ia)
{
	WARN_RATELIMIT(1, "sdcardfs does not support setattr. User setattr2.\n");
	return -EINVAL;
}

static int sdcardfs_setattr(struct vfsmount *mnt, struct user_namespace *mnt_userns, struct dentry *dentry, struct iattr *ia)
{
	int err;
	struct dentry *lower_dentry;
	struct vfsmount *lower_mnt;
